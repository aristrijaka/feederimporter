...
$data = file_get_contents('http://simak.uinsgd.ac.id/gtakademik/services/my_service/feeder_yudisium.php?prodi='.$_GET['jurusan'].'&tahun='.$_GET['tahun']);
...
foreach ($dta->data as $dt) {
	...
	$check = $db->check_exist('mahasiswa',array('id_pd'=>$dt->nipd,));
	if ($check==true)  {
		$sukses++;
	$data = array(		
		  'id_jns_daftar' => $dt->id_jns_daftar,
      'tgl_masuk_sp'=> $dt->tgl_masuk_sp,
      'id_jns_keluar'=> $dt->id_jns_keluar,
      'tgl_keluar'=> $dt->tgl_keluar,
      'ket'=> $dt->ket,
      'skhun'=> $dt->skhun,
      'a_pernah_paud'=> $dt->a_pernah_paud,
      'a_pernah_tk'=> $dt->a_pernah_tk,
      'mulai_smt'=> $dt->mulai_smt.'1',
      'sks_diakui'=> $dt->sks_diakui,
      'jalur_skripsi'=> $dt->jalur_skripsi,
      'judul_skripsi'=> $dt->judul_skripsi,
      'bln_awal_bimbingan'=> $dt->bln_awal_bimbingan,
      'bln_akhir_bimbingan'=> $dt->bln_akhir_bimbingan,
      'sk_yudisium'=> $dt->sk_yudisium,
      'ipk'=> $dt->ipk,
      'status' => 'L',
      'no_seri_ijazah'=> $dt->no_seri_ijazah,
		);
		$in = $db->update('mahasiswa',$data,"id_pd",$dt->nipd);
	}
	else{
	  	$data = array(		
...
if (($sukses>0) || ($error_count>0)) {
	$msg =  "<div class=\"alert alert-warning\" role=\"alert\">
			<font color=\"#3c763d\">".$sukses." data Yudisium baru berhasil Diunduh</font><br />
