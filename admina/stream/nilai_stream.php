...
$data = file_get_contents('http://simak.uinsgd.ac.id/gtakademik/services/my_service/feeder_nilai.php?prodi='.$_GET['jurusan'].'&semester='.$_GET['sem']);
...
foreach ($dta->data as $dt) {
	...
	if ($dt->nilaiHuruf!='') {
			$check = $db->check_exist('nilai',array('nim'=>$dt->nim,'kode_mk' => $dt->kodeMK,'semester'=>$dt->semester,'nama_kelas'=>$dt->kelas));
	if ($check==true) {
		$error_count++;
		$error[] = $dt->nim." ".$dt->kodeMK." Sudah Ada";
	} else {
		$sukses++;
	$data = array(
		'semester' => $dt->semester,
		'nim' => $dt->nim,
		'nama' => $dt->namaMahasiswa,
		'kode_mk' => $dt->kodeMK,
		'nama_mk' => $dt->mataKuliah,
		'nama_kelas' => $nama_kelas,
		'nilai_huruf' => $dt->nilaiHuruf,
		'nilai_indek' => $dt->nilaiBobot,
...
if (($sukses>0) || ($error_count>0)) {
	$msg =  "<div class=\"alert alert-warning\" role=\"alert\">
			<font color=\"#3c763d\">".$sukses." data Nilai berhasil Diunduh</font><br />
