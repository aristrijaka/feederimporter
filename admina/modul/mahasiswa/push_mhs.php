...
$i=1;
	...
	foreach ($arr_data as $value) {
		...
		$filter_nipd = "nipd like '%".$value->nipd."%'";
        $temp_nipd = $proxy->GetRecord($token,'mahasiswa_pt',$filter_nipd);


        if ($temp_nipd['result']) {
           ++$error_count;
           $db->update('mhs_pt',array('status_error' => 2, 'keterangan'=>"Error ".$value->nipd." ".$value->nm_pd." Sudah ada"),'id',$data_mhs_pt->id);
			$error_msg[] = "<b>Error ".$value->nipd." ".$value->nm_pd." Sudah ada di Feeder";
