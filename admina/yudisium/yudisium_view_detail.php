                ...
                <section class="content-header">
                    <h1>
                        Manage Yudisium <?php echo $db->fetch_single_row('jurusan','kode_jurusan',$id_jur)->nama_jurusan;?>
                    </h1>
                        <ol class="breadcrumb">
 ...
 <?php
       foreach ($db->fetch_all("sys_menu") as $isi) {
                    ...
                    ?>
 <button class="btn btn-primary btn-flat down_akm"><i class="fa fa-cloud-download"></i> Download Yudisium Dari Simak</button>
...
<div class="form-group">
                        <label for="Jurusan" class="control-label col-lg-1">Tahun</label>
                        <div class="col-lg-3">
                          <select id="angkatan" name="angkatan" data-placeholder="Pilih Jurusan ..." class="form-control chzn-select" tabindex="2" required>
                        <?php
...
<div class="form-group">
                        <label for="Jurusan" class="control-label col-lg-1">Tahun</label>
                        <div class="col-lg-3">
                          <select id="angkatan_up" name="angkatan" data-placeholder="Pilih Jurusan ..." class="form-control chzn-select" tabindex="2" required>
                        <?php
                        for ($i=date("Y"); $i > 2000 ; $i--) { 
                          echo "<option value='$i'>$i</option>";
                        }
...
<div class="box box-danger">
      ...
      $dtb=$db->fetch_custom("select mahasiswa.nm_pd,mahasiswa.jk,mahasiswa.angkatan,mahasiswa.nik,mahasiswa.id_pd from mahasiswa where kode_prodi=$id_jur and status='L' ");
      $i=1;
      foreach ($dtb as $isi) {
        ?><tr id="line_<?=$isi->id_pd;?>">
        <td align="center"><?=$i;?></td><td><?=$isi->nm_pd;?></td>
<td><?=$isi->jk;?></td>
<td><?=$isi->id_pd;?></td>
<td><?=$isi->angkatan;?></td>

        <td>
        <a href="<?=base_index();?>mahasiswa/detail/<?=$isi->id_pd;?>" class="btn btn-success btn-flat"><i class="fa fa-eye"></i></a> 
        <?=($role_act["up_act"]=="Y")?'<a href="'.base_index().'mahasiswa/edit/'.$isi->id_pd.'" class="btn btn-primary btn-flat"><i class="fa fa-pencil"></i></a>':"";?>  
        <?=($role_act["del_act"]=="Y")?'<button class="btn btn-danger hapus btn-flat" data-uri="'.base_admin().'modul/mahasiswa/mahasiswa_action.php" data-id="'.$isi->id_pd.'"><i class="fa fa-trash-o"></i></button>':"";?>
        </td>
        </tr>
        <?php
        $i++;
...
<input type="hidden" id="semester_selected">
  ...
  $edit = '<a href="'.base_index()."akm/edit/'+aData[indek]+'".'" class="btn btn-primary btn-flat"><i class="fa fa-pencil"></i></a>';
  } else {
    $edit ="";
  }
...
$('.down_akm_now').on('click', function() {
  $("#isi_drop").hide();
  // alert("ll");
   var jur = $("#angkatan").val();
  // alert(jur);


    $.ajax({
     url: "http://103.55.33.23/gtakademik/services/my_service/feeder_yudisium.php?prodi=<?=$id_jur;?>&tahun="+$("#angkatan").val(),
       async: false,
      dataType: 'json',
      success: function(data) {
        jumlah = data.jumlah;
...
window.finished = false;
        $.getJSON('<?=base_admin();?>stream/yudisium_stream.php?tahun='+$("#angkatan").val()+"&total="+jumlah+"&jurusan="+<?=$id_jur;?>,
            function(data){
...
window.finished = false;
        $.getJSON('<?=base_admin();?>stream/push_yudisium.php?angkatan='+$("#angkatan_up").val()+"&jurusan="+<?=$id_jur;?>,
            function(data){
             // alert("lll");
             //   console.log("ALL DONE", data);
                clearInterval(window.progressInterval);
